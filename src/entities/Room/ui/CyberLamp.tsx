    ...
    return (
        <group position={[x, y, z]} rotation={rotation}>
            ...
            <mesh geometry={baseRingGeom} position={[0, -1.22, 0]} rotation={[Math.PI / 2, 0, 0]}>
                <meshStandardMaterial color={COLORS.neonMagenta} emissive={COLORS.neonMagenta} emissiveIntensity={0.6} roughness={0.2} />
            </mesh>
            ...
            {[-0.1, 0.1].map((h, i) => (
                <mesh key={i} geometry={jointGeom} position={[0, h, 0]} castShadow>
                    <meshStandardMaterial color={COLORS.metalLight} roughness={0.3} metalness={0.8} />
                </mesh>
            ))}

            {/* Плафон */}
            <mesh geometry={shadeGeom} position={[0, 0.5, 0]} castShadow>
                <meshStandardMaterial color="#f0f4fa" roughness={0.2} metalness={0.5} transparent opacity={0.85} side={THREE.DoubleSide} />
            </mesh>

            {/* Внутренняя светящаяся сердцевина */}
            <mesh geometry={innerGlowGeom} position={[0, 0.5, 0]}>
                <meshStandardMaterial color={COLORS.neonCyan} emissive={COLORS.neonCyan} emissiveIntensity={0.9} roughness={0.1} metalness={0.1} />
            </mesh>

            {/* Крышки плафона */}
            <mesh geometry={shadeTopGeom} position={[0, 1.4, 0]} castShadow receiveShadow>
                <meshStandardMaterial color={COLORS.surfaceLighter} roughness={0.3} metalness={0.8} />
            </mesh>
            ...
            {/* Неоновые кольца */}
            {[-0.1, 1.2].map((h, i) => (
                <mesh key={i} geometry={neonRingGeom} position={[0, h, 0]} rotation={[Math.PI / 2, 0, 0]}>
                    <meshStandardMaterial color={COLORS.neonCyan} emissive={COLORS.neonCyan} emissiveIntensity={0.8} roughness={0.2} />
                </mesh>
            ))}
            ...
            <mesh position={[0, 1.5, 0]}>
                ...
                <meshStandardMaterial color={COLORS.neonMagenta} emissive={COLORS.neonMagenta} emissiveIntensity={0.9} roughness={0.2} />
            </mesh>
